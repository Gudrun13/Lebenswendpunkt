...
export default function Offers() {
  useSEO({
    title: "Angebote – Begleitung in Trauer, Krisen, Angst & Paarberatung",
    description: "Psychologische Einzelbegleitung und Paarberatung: Trauer, Lebenskrisen, Angst und Umbrüche. Kostenfreies Erstgespräch bei Gudrun Murina, Heilpraktikerin für Psychotherapie.",
    canonical: "https://lebenswendepunkt.de/angebote",
  });
            ...
            <section key={index} className={`py-20 px-4 ${offer.color}`}>
              <div className="container mx-auto max-w-5xl">
                <div className={`flex flex-col md:flex-row gap-10 md:gap-16 items-center ${
                  ...
                  {/* Visual Area */}
                  <div className="w-48 h-48 md:w-64 md:h-64 rounded-full flex items-center justify-center flex-shrink-0 overflow-hidden shadow-sm border-4 border-white/60 bg-white">
                    {offer.image ? (
                      <img src={offer.image} alt={offer.title} className="w-full h-full object-cover" />
                    ) : (
                      offer.icon && <offer.icon className="h-16 w-16 text-primary/40" />
                    )}
                  </div>
